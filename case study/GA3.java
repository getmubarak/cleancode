...
public static void initializePopulation() {
    ...
    for (int chromosome = 0; chromosome < maxPopulation; chromosome++) {
        initializeChromosome(chromosome);
    }
}

// Initializes a single chromosome
private static void initializeChromosome(int chromosome) {
    int[] visitedCities = initializeVisitedCities();
    
    // Set the starting city
    setStartingCity(chromosome);
    
    // Generate cities
    generateCities(chromosome, visitedCities);
    
    // Set the ending city
    setEndingCity(chromosome);
}

// Initialize the visitedCities array
private static int[] initializeVisitedCities() {
    int[] visitedCities = new int[totalCities + 1];
    Arrays.fill(visitedCities, -1);
    return visitedCities;
}

// Set the starting city in the chromosome
private static void setStartingCity(int chromosome) {
    population[generation][chromosome][0] = startCity;
    System.out.print(generation + "-" + chromosome + " : " + startCity);
}

// Generate the cities for the chromosome
private static void generateCities(int chromosome, int[] visitedCities) {
    for (int gene = 1; gene <= totalCities; gene++) {
        int city;
        do {
            city = genRandom(totalCities + 1);
        } while (isCityVisited(city, visitedCities, startCity));
...
// Set the ending city in the chromosome
private static void setEndingCity(int chromosome) {
    population[generation][chromosome][totalCities + 1] = startCity;
    System.out.println(" " + startCity);
}

// Helper method to check if a city is already visited
private static boolean isCityVisited(int city, int[] visitedCities, int startCity) {
    if (city == startCity) return true;
    for (int visitedCity : visitedCities) {
        if (city == visitedCity) return true;
    }
    return false;
}
